...
export default function DealDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Link
        href="/deals"
        className="inline-flex items-center gap-2 text-sm text-cascadia-500 hover:text-cascadia-700 transition-colors"
      >
      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-2xl font-semibold text-cascadia-900">{deal.name}</h1>
            <Badge variant="success">Reviewed</Badge>
          </div>
          <div className="flex flex-wrap items-center gap-4 text-sm text-cascadia-500">
      ...
      {/* Tab Navigation */}
      <div className="border-b border-cascadia-200">
            ...
            <Card>
              ...
              <CardContent>
                <ValueRange
                  ...
                  compact
                />
                <div className="mt-6 pt-6 border-t border-cascadia-200">
                  <OfferGuidance
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div>
                    <p className="text-sm text-cascadia-500">Occupancy</p>
                    <p className="text-xl font-semibold text-cascadia-900">
                      {formatPercent(deal.census.occupancy, 0)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-cascadia-500">EBITDAR Margin</p>
                    <p className="text-xl font-semibold text-cascadia-900">
                      {formatPercent(deal.financials.ebitdarMargin, 0)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-cascadia-500">Agency Labor</p>
                    <p className="text-xl font-semibold text-status-warning">
                      {formatPercent(deal.financials.agencyPercent, 0)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-cascadia-500">CMS Rating</p>
                    <p className="text-xl font-semibold text-cascadia-900">
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-4 gap-4">
                  <div className="text-center p-4 bg-cascadia-50 rounded-lg">
                    <p className="text-2xl font-bold text-cascadia-900">{deal.regulatory.cmsRating}</p>
                    <p className="text-xs text-cascadia-500 mt-1">Overall</p>
                  </div>
                  <div className="text-center p-4 bg-cascadia-50 rounded-lg">
                    <p className="text-2xl font-bold text-cascadia-900">{deal.regulatory.healthRating}</p>
                    <p className="text-xs text-cascadia-500 mt-1">Health</p>
                  </div>
                  <div className="text-center p-4 bg-yellow-50 rounded-lg">
                    <p className="text-2xl font-bold text-status-warning">{deal.regulatory.staffingRating}</p>
                    <p className="text-xs text-cascadia-500 mt-1">Staffing</p>
                  </div>
                  <div className="text-center p-4 bg-cascadia-50 rounded-lg">
                    <p className="text-2xl font-bold text-cascadia-900">{deal.regulatory.qualityRating}</p>
                    <p className="text-xs text-cascadia-500 mt-1">Quality</p>
                  </div>
                </div>
                <p className="mt-4 text-sm text-cascadia-500">
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-cascadia-600">Immediate</span>
                    <span className="font-medium text-status-error">{formatCurrency(deal.capEx.immediate)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-cascadia-600">Deferred</span>
                    <span className="font-medium text-status-warning">{formatCurrency(deal.capEx.deferred)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-cascadia-600">Competitive</span>
                    <span className="font-medium text-cascadia-700">{formatCurrency(deal.capEx.competitive)}</span>
                  </div>
                  <div className="flex justify-between items-center pt-3 border-t border-cascadia-200">
                    <span className="font-medium text-cascadia-900">Total</span>
                    <span className="font-bold text-cascadia-900">{formatCurrency(deal.capEx.total)}</span>
                  </div>
                  <p className="text-xs text-cascadia-500">
              ...
              <CardContent>
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    <span className="text-cascadia-500">Address</span>
                    <span className="text-cascadia-700 text-right">{deal.address}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-cascadia-500">Year Built</span>
                    <span className="text-cascadia-700">{deal.yearBuilt}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-cascadia-500">Last Renovation</span>
                    <span className="text-cascadia-700">{deal.lastRenovation}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-cascadia-500">Square Footage</span>
                    <span className="text-cascadia-700">{deal.sqft.toLocaleString()} SF</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-cascadia-500">Broker</span>
                    <span className="text-cascadia-700">{deal.broker}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card variant="external">
              <CardHeader>
                <div className="flex items-center gap-2">
                  <ExternalLink className="w-5 h-5 text-cascadia-500" />
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-cascadia-200">
                  <div>
                    <p className="text-sm text-cascadia-500">Cap Rate Range</p>
                    <p className="font-medium text-cascadia-900">
                      {formatPercent(deal.externalValuation.capRateHigh, 1)} - {formatPercent(deal.externalValuation.capRateLow, 1)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-cascadia-500">NOI Used</p>
                    <p className="font-medium text-cascadia-900">
                      {formatCurrency(deal.externalValuation.noiUsed)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-cascadia-500">Price/Bed</p>
                    <p className="font-medium text-cascadia-900">
                      {formatCurrency(deal.externalValuation.pricePerBed)}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-cascadia-500">Confidence</p>
                    <p className="font-medium text-cascadia-900">
                ...
                <p className="text-sm text-cascadia-600 italic">
              ...
              <CardContent className="space-y-6">
                ...
                {deal.cascadiaValuation.upsideScenario && (
                  <div className="p-4 rounded-lg bg-accent/5 border border-accent/20">
                    ...
                    <p className="text-sm text-cascadia-700">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="text-center p-6 rounded-lg bg-accent/5 border border-accent/20">
                  <p className="text-sm text-cascadia-500 mb-2">Suggested Starting Offer</p>
                  <p className="text-3xl font-bold text-accent">{formatCurrency(deal.suggestedOffer, true)}</p>
                  <p className="text-xs text-cascadia-500 mt-2">Conservative entry point</p>
                </div>
                <div className="text-center p-6 rounded-lg bg-status-warning/5 border border-status-warning/20">
                  <p className="text-sm text-cascadia-500 mb-2">Walk-Away Threshold</p>
                  <p className="text-3xl font-bold text-status-warning">{formatCurrency(deal.walkAwayThreshold, true)}</p>
                  <p className="text-xs text-cascadia-500 mt-2">Maximum defensible price</p>
                </div>
                <div className="text-center p-6 rounded-lg bg-cascadia-50">
                  <p className="text-sm text-cascadia-500 mb-2">Asking Price</p>
                  <p className="text-3xl font-bold text-cascadia-700">{formatCurrency(deal.askingPrice, true)}</p>
                  <p className="text-xs text-cascadia-500 mt-2">Broker listed price</p>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                <TableBody>
                  {deal.partnerMatches.map((partner, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-2 rounded-full bg-cascadia-200 overflow-hidden">
                            <div
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between py-2">
                  <span className="text-sm text-cascadia-600">Total Revenue</span>
                  <span className="text-xl font-bold text-cascadia-900">
                    {formatCurrency(deal.financials.totalRevenue)}
                  </span>
                </div>
                <div className="space-y-3 pt-4 border-t border-cascadia-200">
                  {[
                    ...
                    <div key={item.label} className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-cascadia-600">{item.label}</span>
                        <span className="font-medium text-cascadia-900">{formatCurrency(item.value)}</span>
                      </div>
                      <div className="h-2 rounded-full bg-cascadia-200 overflow-hidden">
                        <div
                          className={`h-full rounded-full ${item.color}`}
                          style={{ width: `${item.percent * 100}%` }}
                        />
                      </div>
                      <p className="text-xs text-cascadia-500 text-right">{formatPercent(item.percent, 0)}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between py-2">
                  <span className="text-sm text-cascadia-600">Total Labor Cost</span>
                  <span className="text-xl font-bold text-cascadia-900">
                ...
                <div className="space-y-3 pt-4 border-t border-cascadia-200">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-cascadia-600">Core Labor</span>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-cascadia-600">Agency Labor</span>
                ...
                <div className="p-4 rounded-lg bg-status-warning/10 border border-status-warning/20">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-cascadia-700">Agency % of Nursing</span>
                    <span className="text-lg font-bold text-status-warning">
                      {formatPercent(deal.financials.agencyPercent, 0)}
                    </span>
                  </div>
                  <p className="text-xs text-cascadia-500 mt-2">
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="flex items-center justify-between py-3 border-b border-cascadia-200">
                  <span className="text-sm text-cascadia-600">EBITDAR</span>
                  <div className="text-right">
                    <span className="text-xl font-bold text-cascadia-900">
                      {formatCurrency(deal.financials.ebitdar)}
                    </span>
                    <span className="text-sm text-cascadia-500 ml-2">
                      ({formatPercent(deal.financials.ebitdarMargin, 0)} margin)
                    </span>
                  </div>
                </div>
                <div className="flex items-center justify-between py-3">
                  <span className="text-sm text-cascadia-600">Normalized NOI</span>
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center p-4 rounded-lg bg-cascadia-50">
                    <p className="text-3xl font-bold text-cascadia-900">{deal.census.averageDailyCensus}</p>
                    <p className="text-xs text-cascadia-500 mt-1">Average Daily Census</p>
                  </div>
                  <div className="text-center p-4 rounded-lg bg-accent/10">
                    <p className="text-3xl font-bold text-accent">{formatPercent(deal.census.occupancy, 0)}</p>
                    <p className="text-xs text-cascadia-500 mt-1">Occupancy Rate</p>
                  </div>
                </div>
          ...
          <Card>
            <CardContent className="py-8">
              <div className="text-center max-w-2xl mx-auto">
                <h2 className="text-2xl font-bold text-cascadia-900 mb-2">{deal.name}</h2>
                <p className="text-cascadia-500 mb-6">
                  Investment Committee Summary | {deal.assetType} | {deal.beds} Beds | {deal.city}, {deal.state}
                </p>
                <div className="grid grid-cols-3 gap-8">
                  <div>
                    <p className="text-sm text-cascadia-500">Suggested Offer</p>
                    <p className="text-2xl font-bold text-accent">{formatCurrency(deal.suggestedOffer, true)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-cascadia-500">Walk-Away</p>
                    <p className="text-2xl font-bold text-status-warning">{formatCurrency(deal.walkAwayThreshold, true)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-cascadia-500">Confidence</p>
                    <p className="text-2xl font-bold text-cascadia-900">{deal.cascadiaValuation.confidenceScore}/100</p>
                  </div>
                </div>
              </div>
            </CardContent>
              ...
              <CardContent>
                <ul className="space-y-3">
                  {deal.whatMustGoRightFirst.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-sm text-cascadia-700">
