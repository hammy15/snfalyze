...
export function PipelineResults({
  ...
  const recommendationConfig = {
    pursue: { label: 'Pursue', color: 'text-emerald-700 dark:text-emerald-400', bg: 'bg-emerald-100 border-emerald-300 dark:bg-emerald-500/10 dark:border-emerald-500/30' },
    conditional: { label: 'Conditional', color: 'text-amber-700 dark:text-amber-400', bg: 'bg-amber-100 border-amber-300 dark:bg-amber-500/10 dark:border-amber-500/30' },
    pass: { label: 'Pass', color: 'text-rose-700 dark:text-rose-400', bg: 'bg-rose-100 border-rose-300 dark:bg-rose-500/10 dark:border-rose-500/30' },
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      {/* Hero Score Card */}
      <div className="relative rounded-2xl border border-surface-200 dark:border-surface-700/50 bg-white dark:bg-surface-900/50 shadow-sm p-6 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-primary-50 dark:from-primary-500/5 to-transparent" />
        <div className="relative flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-surface-900 dark:text-surface-100 mb-1">{dealName}</h2>
        ...
        {/* Investment Thesis */}
        <div className="relative mt-4 p-3 rounded-xl bg-surface-50 dark:bg-surface-800/50 border border-surface-200 dark:border-surface-700/30">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {/* Valuation Summary */}
        <div className="rounded-xl border border-surface-200 dark:border-surface-700/50 bg-white dark:bg-surface-900/30 p-4 shadow-sm">
          ...
          <div className="space-y-1.5 max-h-40 overflow-y-auto">
            {criticalFlags.map((flag) => (
              <div key={flag.id} className="flex items-start gap-2 text-xs">
                <AlertTriangle className="w-3 h-3 text-rose-500 dark:text-rose-400 mt-0.5 flex-shrink-0" />
          ...
          <div className="space-y-1.5">
            {synthesis.toolSummary.map((tool, i) => (
              <div key={i} className="flex items-start gap-2 text-xs">
                <TrendingUp className="w-3 h-3 text-primary-600 dark:text-primary-400 mt-0.5 flex-shrink-0" />
                <span className="text-surface-700 dark:text-surface-300">{tool.headline}</span>
              </div>
            ))}
      ...
      <div className="rounded-xl border border-surface-200 dark:border-surface-700/50 bg-white dark:bg-surface-900/30 p-4 shadow-sm">
        ...
        <div className="space-y-1.5">
          {synthesis.suggestedNextSteps.map((step, i) => (
            <div key={i} className="flex items-center gap-2 text-xs text-surface-700 dark:text-surface-300">
              <div className="w-4 h-4 rounded-full bg-surface-100 dark:bg-surface-800 flex items-center justify-center flex-shrink-0">
