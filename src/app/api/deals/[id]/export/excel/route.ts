...
import {
  ...
  type ExportInput,
} from '@/lib/export/excel/workbook-builder';
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    builder.buildSaleLeasebackWorkbook(exportInput);
    const buffer = builder.exportToBuffer();

    // Generate filename
    const sanitizedName = deal.name.replace(/[^a-z0-9]/gi, '_').toLowerCase();
    const timestamp = new Date().toISOString().split('T')[0];
    const filename = `${sanitizedName}_sale_leaseback_${timestamp}.xlsx`;

    // Return file response - convert Buffer to Uint8Array for NextResponse compatibility
