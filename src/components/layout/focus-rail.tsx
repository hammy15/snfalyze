    ...
    <>
      ...
      >
        ...
        <Link
          href="/app"
          className={cn(
            'flex items-center gap-3 px-3 h-14 border-b border-surface-800 group',
        ...
        >
          ...
          {expanded && (
            <span className="text-white font-semibold text-sm animate-fade-in">SNFalyze</span>
          )}
        </Link>
        ...
        <div className="flex-1 py-3 space-y-1 px-2">
          {visibleItems.map(item => {
            ...
            return (
              ...
              >
                {active && (
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1 w-1 h-5 bg-primary-400 rounded-r-full" />
                )}
                ...
                {!expanded && (
                  <div className="absolute left-full ml-2 px-2 py-1 bg-surface-800 text-surface-200 text-xs rounded-md opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-50 shadow-lg">
                    {item.label}
                  </div>
                )}
              </Link>
              ...
              {recentDeals.slice(0, expanded ? 4 : 3).map(deal => (
                <Link
                  ...
                  className={cn(
                    ...
                    expanded
                      ? 'flex items-center gap-2 px-2 py-1.5 rounded-lg text-surface-400 hover:text-surface-200 hover:bg-surface-800/70'
                      : 'block'
                  )}
                >
                  <div className={cn(
        ...
        {/* User */}
        <div className="px-2 py-3 border-t border-surface-800 relative">
          <button
            onClick={() => setShowUserMenu(!showUserMenu)}
            className={cn(
              'flex items-center gap-3 w-full px-2 py-2 rounded-lg transition-colors',
              'text-surface-400 hover:text-surface-200 hover:bg-surface-800/70',
              showUserMenu && 'bg-surface-800/70 text-surface-200'
            )}
          >
            ...
            {expanded && (
              <div className="flex-1 min-w-0 text-left animate-fade-in">
                ...
                <div className="text-[10px] text-surface-500 capitalize">{user?.role}</div>
              </div>
            )}
          </button>
          ...
          {showUserMenu && expanded && (
            <div className="absolute bottom-full left-2 right-2 mb-1 bg-surface-800 border border-surface-700 rounded-lg shadow-xl overflow-hidden z-50 animate-scale-in">
              <Link
                href="/settings"
                className="flex items-center gap-2 px-3 py-2 text-xs text-surface-300 hover:bg-surface-700 transition-colors"
              >
              ...
              {(user?.role === 'admin' || user?.role === 'vp') && (
                <Link
                  href="/app/admin"
                  className="flex items-center gap-2 px-3 py-2 text-xs text-amber-400 hover:bg-surface-700 transition-colors"
                >
              ...
              <button
                onClick={logout}
                className="flex items-center gap-2 w-full px-3 py-2 text-xs text-rose-400 hover:bg-surface-700 transition-colors"
              >
